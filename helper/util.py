...
# Assuming FLOW_FILE_EXTENSION and BUFFER_FILE_EXTENSION are defined somewhere above

def open_compressed_file(path, write=False):
    """
    Opens a file (possibly compressed) in the correct mode for Python 3.
    Text files (.csv, .gz, .bz2) are opened in text mode with UTF-8 encoding.
    Binary files (.pcap, etc.) are opened in binary mode.
    """
    file_extension = os.path.splitext(path)[1].replace('.', '')

    # --- Handle GZIP files (.gz)
    if file_extension == 'gz':
        mode = 'wb' if write else 'rb'
        f = gzip.open(path, mode)

    # --- Handle BZIP2 files (.bz2)
    elif file_extension == 'bz2':
        mode = 'wb' if write else 'rb'
        f = bz2.open(path, mode)

    # --- Handle CSV / PCAP / FLOW / BUFFER files
    elif file_extension in ['csv', 'pcap', FLOW_FILE_EXTENSION, BUFFER_FILE_EXTENSION]:
        # PCAP and binary-like files → binary mode
        if file_extension == 'pcap':
            mode = 'wb' if write else 'rb'
            f = open(path, mode)
        else:
            # CSV / flow / buffer files → text mode with UTF-8 encoding
