...
from helper import CSV_PATH, CSV_FILE_NAMES, INFORMATION_FILE, COMPRESSION_EXTENSIONS
from helper.util import open_compressed_file, find_file


# ----------------------------------------------------------------------
# Write all CSVs for a given PcapData
# ----------------------------------------------------------------------
def write_to_csv(path, pcap_data, compression):
    ...
    for value in value_dict:
        write_csv(
            os.path.join(path, CSV_FILE_NAMES[value]),
            value_dict[value],
...
# ----------------------------------------------------------------------
# Write a single CSV file (supports gzip, bz2, or plain text)
# ----------------------------------------------------------------------
def write_csv(path, data, compression):
    ext = COMPRESSION_EXTENSIONS.get(compression, '')

    if compression == 'gzip':
        f = gzip.open(f"{path}{ext}", 'wt', encoding='utf-8')
    elif compression == 'bz2':
        f = bz2.open(f"{path}{ext}", 'wt', encoding='utf-8')
    else:
        f = open(f"{path}{ext}", 'w', encoding='utf-8')

    connections = []
    max_length = 0
    columns = 1

    # --- Write connection IDs as header ---
    for d in data:
...
# ----------------------------------------------------------------------
# Read back CSVs into a PcapData structure
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# Generic CSV reader (auto-detect compressed files)
# ----------------------------------------------------------------------
def read_csv(path, columns_per_connection=2):
    ...
    first_line = f.readline().decode('utf-8').split(';')[:-1]
    ...
    for line in f:
        ...
        for i in range(0, len(first_line), columns_per_connection):
            ...
            if index not in output:
                output[index] = tuple([[] for _ in range(columns_per_connection)])

            for column in range(columns_per_connection):
                try:
...
# ----------------------------------------------------------------------
# Write info summary file (human-readable metrics)
# ----------------------------------------------------------------------
def write_info_file(path, pcap_data):
    ...
    with open(path, 'w', encoding='utf-8') as f:
        f.write(f"Synchronized at:\n {pcap_data.data_info.sync_phases}\n")
        f.write(f"with durations:\n {pcap_data.data_info.sync_duration}\n")

        sync_duration = pcap_data.data_info.sync_duration
        if len(sync_duration) > 0:
            f.write(f"Avg:\n {sum(sync_duration) / len(sync_duration)}\n")
        else:
            f.write("Avg:\n 0\n")
        ...
        for percentage in percentages:
            f.write(f"\n{'' :-<58}\n\nValues used: last {percentage * 100}%\n")

            for d in data_values:
                f.write(f"\n{d[0]}:\n")
                f.write("{:13}  {:>13}  {:>13}  {:>13}  {:>13}  {:>13}\n".format(
                    'Connection', 'Median', 'Mean', 'Std Dev', 'Min', 'Max'
                ))
